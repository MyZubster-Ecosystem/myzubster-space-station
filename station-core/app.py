...
def paths(rule):
    def deco(fn):
        app.add_url_rule('/api'+rule, fn.__name__+'_api', fn, methods=getattr(fn,'methods',['GET']))
        app.add_url_rule(rule, fn.__name__+'_direct', fn, methods=getattr(fn,'methods',['GET']))
        return fn
    return deco

def methods(*m):
    def deco(fn): fn.methods=list(m); return fn
    return deco
...
@app.route('/health')
def health_direct(): return health()

@app.route('/api/robots');
def get_robots(): return jsonify(list(robots.values()))
@app.route('/robots')
def get_robots_direct(): return get_robots()

@app.route('/api/missions', methods=['GET','POST'])
def missions_api():
    if request.method=='GET': return jsonify(missions)
    data=request.json or {}; mission={"id":str(uuid.uuid4()),"type":data.get('type','environmental_scan'),"target":data.get('target','green-module'),"robot_id":data.get('robot_id'),"status":"CREATED","created_at":datetime.utcnow().isoformat()}; missions.append(mission); return jsonify(mission),201
@app.route('/missions', methods=['GET','POST'])
def missions_direct(): return missions_api()

@app.route('/api/telemetry', methods=['GET','POST'])
def telemetry_api():
    if request.method=='GET': return jsonify(telemetry)
    data=request.json or {}; point={"id":str(uuid.uuid4()),"robot_id":data.get('robot_id'),"timestamp":datetime.utcnow().isoformat(),"temperature":data.get('temperature'),"humidity":data.get('humidity'),"battery":data.get('battery'),"location":data.get('location')}; telemetry.append(point); return jsonify(point),201
@app.route('/telemetry', methods=['GET','POST'])
def telemetry_direct(): return telemetry_api()

@app.route('/api/payments', methods=['GET','POST'])
def payments_api():
    if request.method=='GET': return jsonify(payments)
    data=request.json or {}; payment={"id":str(uuid.uuid4()),"amount":data.get('amount'),"currency":data.get('currency','MYZ'),"purpose":data.get('purpose','Mission payment'),"status":"COMPLETED","timestamp":datetime.utcnow().isoformat(),"transaction_id":str(uuid.uuid4())[:8]}; payments.append(payment); return jsonify(payment),201
@app.route('/payments', methods=['GET','POST'])
def payments_direct(): return payments_api()
